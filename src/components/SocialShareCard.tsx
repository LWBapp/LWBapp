...
const SocialShareCard = React.forwardRef<HTMLDivElement, Props>(
  ({ country, description, archetype }, ref) => {
    return (
      <div
      ...
      >
        <h2 className="text-[2.2rem] font-playfair text-honey-dark font-bold mb-6 text-center leading-snug drop-shadow-lg">
          Life Without Borders
        </h2>
        <div className="flex flex-col items-center bg-white/90 rounded-xl px-8 py-8 w-full">
          {/* Archetype Title */}
          {archetype && (
            <div className="text-xl md:text-2xl font-bold text-soul-purple text-center mb-2">
              {archetype}
            </div>
          )}
          <div className="text-md md:text-lg font-semibold text-blush-dark text-center mb-1">
            Your Soul Country
          </div>
          <div className="text-[2.2rem] md:text-[2.7rem] leading-tight font-playfair italic font-bold text-ocean-dark mb-4 text-center">
            {country}
          </div>
          <div className="w-10 h-1 bg-honey-dark rounded-full mb-5" />
          {/* Vibe quote or description (poetic block) */}
          <p className="text-base text-gray-700 text-center whitespace-pre-line leading-normal mb-2 font-serif">
            {description}
          </p>
        </div>
